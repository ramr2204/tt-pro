...
class Declaraciones extends MY_Controller
{
    public function __construct()
    {
        ...
        $this->load->helper(array('form', 'url', 'codegen_helper', 'array'));
        $this->load->helper(array('Equivalencias', 'EquivalenciasFirmas', 'EquivalenciasNotificaciones'));
    ...
    public function create()
    {
        if ($this->ion_auth->logged_in())
        {
            ...
            {
                $_POST = array_merge($_POST, ($this->session->flashdata('campos') ? $this->session->flashdata('campos') : array()));
    ...
    private function validacionInicial()
    {
        ...
        {
            $respuesta = array(false, (validation_errors() ? validation_errors() : false));
        }
    ...
    public function generarPdf($id_declaracion, $datos_firma, $info)
    {
        ...
        $this->data['firmas']      = $info['info'] ? $info['info'] : array();
    ...
    public function cargarPago()
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if (!is_dir($path))
                { //crea la carpeta para los objetos si no existe
    ...
    public function solicitarCorreccion()
    {
        ...
        else
        {
            ...
            if (!$verificacion)
            {
                ...
                {
                    $guardo = $this->codegen_model->add('correcciones_declaraciones', array(
                        'id_declaracion'      => $this->input->post('declaracion'),
                    ...
                    if ($guardo->bandRegistroExitoso)
                    {
                        $this->codegen_model->edit(
                            'declaraciones',
                            array('estado' => EquivalenciasFirmas::declaracionSolicitadaCorreccion()),
                            'id', $this->input->post('declaracion')
                        );

                        $this->codegen_model->add('notificaciones', array(
            ...
            if ($this->ion_auth->is_admin() || $this->ion_auth->in_menu('declaraciones/contestarCorreccion'))
            {
                ...
                else
                {
                    ...
                    if ($solicitudCorreccion)
                    {
                        ...
                        $this->codegen_model->edit(
                            'declaraciones',
                            array('estado' => $estadoDeclaracion),
                            'id', $solicitudCorreccion->id_declaracion
                        );
                    ...
                    if (!$verificacion)
                    {
                        ...
                        if ($respuesta['mensaje'] === '')
                        {
                            ...
                            if ($guardo->bandRegistroExitoso)
                            {
                                $this->codegen_model->edit(
                                    'declaraciones',
                                    array('estado' => EquivalenciasFirmas::declaracionCorregida()),
                                    'id', $this->input->post('declaracion')
                                );
            ...
            if ($this->ion_auth->is_admin() || $this->ion_auth->in_menu('declaraciones/comprobar'))
            {
                ...
                else
                {
                    ...
                    if ($declaracion->estado == EquivalenciasFirmas::declaracionPagada())
                    {
                        ...
                        if (!$acepto)
                        {
                            $this->codegen_model->add('correcciones_declaraciones', array(
                                'id_declaracion'      => $this->input->post('declaracion'),
                                'id_usuario_verifico' => $this->session->userdata('user_id'),
                                'estado'              => EquivalenciasFirmas::correccionAceptada(),
                                'fecha_creacion'      => date('Y-m-d H:i:s'),
                        ...
                        $this->codegen_model->edit(
                            'declaraciones',
                            array('estado' => ($acepto ? EquivalenciasFirmas::declaracionAceptada() : EquivalenciasFirmas::declaracionRechazada())),
                            'id', $this->input->post('declaracion')
                        );
